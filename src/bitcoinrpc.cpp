...
{
    return GetBoolArg("-testnet", false) ? 56548 : 4525;
}
...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop Shopzcoin server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Shopzcoin server stopping";
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
    ...
    { "sendshopzcoinddress",          &sendshopzcoinddress,          false,  false },
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: shopzcoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("shopzcoin-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use shopzcoind";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("shopzcoin-rpchand");
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "sendshopzcoinddress"          && n > 1) ConvertTo<double>(params[1]);
